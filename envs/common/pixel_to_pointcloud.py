...
# Example: All joints at zero (neutral position)
print("\n" + "="*60)
print("JOINT ORIENTATIONS - NEUTRAL POSITION")
print("="*60)

neutral_angles = {joint_name: -0.105 for joint_name in movable_joints}
...
print("\n" + "="*60)
print("LOADING REAL TOF DATA")
print("="*60)

# Load real ToF data
real_tof_path = '/home/generalroboticslab/repo/Argus_Boxi/vrobot_env/envs/common/tof_data_20250619_223633.npy'

try:
    # Load real sensor data and downsample to 5x5 using min pooling
    real_distances = load_real_tof_data(real_tof_path, frame_index=0, expected_sensors=20, pooling_method='center')
    print(f"Successfully loaded real ToF data: {real_distances.shape}")
    
    # Extract joint origins, directions, and quaternions
    joint_origins = []
    joint_directions = []
    joint_quaternions = []
    ...
    num_sensors = min(len(joint_origins), len(real_distances))
    if num_sensors < 20:
        print(f"WARNING: Only using {num_sensors} sensors (expected 20)")
    
    # Trim data to match available joints
    joint_origins = joint_origins[:num_sensors]
    joint_directions = joint_directions[:num_sensors]
    joint_quaternions = joint_quaternions[:num_sensors]
    real_distances = real_distances[:num_sensors]

    # Convert ray distances to 3D positions using real data
    point_positions = convert_ray_distance_to_position(
        origins=joint_origins,
        directions=joint_directions, 
        quaternions=joint_quaternions,
    ...
    print("\n" + "="*60)
    print("3D VISUALIZATION WITH REAL TOF DATA")
    print("="*60)
    ...
    # Plot all point positions colored by distance from origin
    distances_from_origin = np.linalg.norm(point_positions, axis=1)
    scatter = ax.scatter(point_positions[:, 0], point_positions[:, 1], point_positions[:, 2], 
              c=distances_from_origin, cmap='viridis', s=30, alpha=0.7, label='Real ToF Points')
    
    # Add colorbar
    plt.colorbar(scatter, ax=ax, label='Distance from Origin (m)', shrink=0.5)
    
    # Plot joint origins for reference
    joint_origins_array = np.array(joint_origins)
    ax.scatter(joint_origins_array[:, 0], joint_origins_array[:, 1], joint_origins_array[:, 2],
              c='red', s=100, marker='o', label='Joint Origins', alpha=0.9)
    
    # Draw lines from joint origins to their corresponding ray endpoints
    colors = plt.cm.tab20(np.linspace(0, 1, num_sensors))
    
    for joint_idx in range(num_sensors):
        start_idx = joint_idx * 25  # 25 rays per sensor
        end_idx = start_idx + 25
        joint_rays = point_positions[start_idx:end_idx]
        joint_origin = joint_origins[joint_idx]
        
        # Draw a few representative rays (not all 25 to avoid clutter)
        ray_indices = [0, 6, 12, 18, 24]  # corners and center
        for ray_idx in ray_indices:
            if ray_idx < len(joint_rays):
                ray_point = joint_rays[ray_idx]
                ax.plot([joint_origin[0], ray_point[0]], 
                       [joint_origin[1], ray_point[1]], 
                       [joint_origin[2], ray_point[2]], 
                       color=colors[joint_idx % len(colors)], alpha=0.4, linewidth=1)
    
    # Plot joint Z-direction vectors (sensor pointing directions)
    for joint_idx, (origin, direction) in enumerate(zip(joint_origins, joint_directions)):
        end_point = np.array(origin) + np.array(direction) * 0.3  # Scale for visibility
        ax.quiver(origin[0], origin[1], origin[2],
                 direction[0], direction[1], direction[2],
                 color=colors[joint_idx % len(colors)], arrow_length_ratio=0.1, 
    ...
    ax.set_ylabel('Y (m)')
    ax.set_zlabel('Z (m)')
    ax.set_title(f'Real ToF Data Visualization - Min Pooling ({num_sensors} Sensors, {len(point_positions)} Points)')
    ax.legend()
    
    # Set equal aspect ratio
    all_points = np.vstack([joint_origins_array, point_positions])
    ...
    # Print statistics
    print(f"\nReal Data Visualization Statistics:")
    print(f"Total points visualized: {len(point_positions)}")
    print(f"Sensors used: {num_sensors}")
    print(f"Points per sensor: 25 (5x5 downsampled with min pooling)")
    print(f"Point cloud bounds:")
    print(f"  X: [{point_positions[:, 0].min():.3f}, {point_positions[:, 0].max():.3f}]")
    print(f"  Y: [{point_positions[:, 1].min():.3f}, {point_positions[:, 1].max():.3f}]")
    print(f"  Z: [{point_positions[:, 2].min():.3f}, {point_positions[:, 2].max():.3f}]")
    print(f"Distance range: [{distances_from_origin.min():.3f}, {distances_from_origin.max():.3f}] meters")
    
except Exception as e:
    print(f"Error loading real ToF data: {e}")
    print("Falling back to fake data for demonstration...")
    ...
    for joint_name in movable_joints:
        if joint_name in orientations_neutral:
            ...
            joint_origins.append(data['position_xyz'])
            joint_directions.append(data['z_direction'])
            joint_quaternions.append(data['quaternion'])

    num_joints = len(joint_origins)
    fake_distances = np.ones((num_joints, 25))  # All distances = 1.0

    point_positions = convert_ray_distance_to_position(
        origins=joint_origins,
        directions=joint_directions, 
        quaternions=joint_quaternions,
        distance_pixel_normalized=fake_distances,
        resolution=[5, 5]
    )
    
    print(f"Using fake data - Generated point positions shape: {point_positions.shape}")
...
# ===================================================================
# POINT CLOUD PROCESSING WITH CENTERING
# ===================================================================

# After you have generated point_positions, add this processing:

print("\n" + "="*60)
print("POINT CLOUD PROCESSING AND CENTERING")
print("="*60)

# # Method 1: Simple filtering and centering
# print("\nMethod 1: Simple filtering and centering")
# filtered_points, valid_indices, translation = filter_and_center_point_cloud(
#     point_positions, 
#     distance_threshold=2.0,  # Keep points within 3 meters
#     centering_method='centroid'  # Use centroid for centering
# )

# Method 2: Complete processing pipeline
print("\nMethod 2: Complete processing pipeline")


processed_points, processing_info = process_point_cloud_complete(
    point_positions,
    distance_threshold=1.5,  # Use adaptive threshold
    centering_method='centroid',
    remove_outliers=True,
    outlier_multiplier=1.5
)
# ===================================================================
# UPDATED 3D VISUALIZATION WITH PROCESSED DATA
# ===================================================================

print("\n" + "="*60)
print("3D VISUALIZATION WITH PROCESSED POINT CLOUD")
print("="*60)
...
distances_original = np.linalg.norm(point_positions, axis=1)
scatter1 = ax1.scatter(point_positions[:, 0], point_positions[:, 1], point_positions[:, 2], 
                      c=distances_original, cmap='viridis', s=20, alpha=0.7)
ax1.set_title(f'Original Point Cloud ({len(point_positions)} points)')
ax1.set_xlabel('X (m)')
ax1.set_ylabel('Y (m)')
ax1.set_zlabel('Z (m)')
...
# Plot joint origins for reference
joint_origins_array = np.array(joint_origins)
ax1.scatter(joint_origins_array[:, 0], joint_origins_array[:, 1], joint_origins_array[:, 2],
           c='red', s=100, marker='o', label='Joint Origins', alpha=0.9)

# Plot 2: Processed data
if len(processed_points) > 0:
    distances_processed = np.linalg.norm(processed_points, axis=1)
    scatter2 = ax2.scatter(processed_points[:, 0], processed_points[:, 1], processed_points[:, 2], 
                          c=distances_processed, cmap='viridis', s=20, alpha=0.7)
    ax2.set_title(f'Processed Point Cloud ({len(processed_points)} points)')
    
    # Add origin marker
    ax2.scatter([0], [0], [0], c='red', s=200, marker='x', label='Origin [0,0,0]', linewidth=3)
else:
    ax2.set_title('Processed Point Cloud (No points remaining)')
...
print(f"Original points: {len(point_positions)}")
print(f"Processed points: {len(processed_points)}")
if len(processed_points) > 0:
    print(f"New center of mass: {np.mean(processed_points, axis=0)}")
    print(f"Distance from origin range: [{np.linalg.norm(processed_points, axis=1).min():.3f}, {np.linalg.norm(processed_points, axis=1).max():.3f}] m")



# def transform_to_world_coordinates(points, joint_origins, robot_base_height=None):
#     """
#     Transform points from robot coordinate system to world coordinate system.
#     World coordinates have origin at the bottom center of the robot on the ground.
    
#     Args:
#         points: numpy array of shape (N, 3) - points in robot coordinates
#         joint_origins: list of joint origins to help determine robot bounds
#         robot_base_height: explicit height offset (if None, calculated from joint positions)
    
#     Returns:
#         tuple: (world_points, transformation_info)
#     """
#     print("\n" + "="*60)
...
#     # Convert joint origins to numpy array for easier processing
#     joint_origins_array = np.array(joint_origins)
    
#     # Method 1: Calculate robot base height from joint positions
#     if robot_base_height is None:
#         # Find the lowest Z coordinate among all joints (this should be near the robot base)
#         min_joint_z = np.min(joint_origins_array[:, 2])
        
#         # For Argus robot, the base_link is typically at the bottom
#         # We'll use the minimum Z value as the robot base reference
#         robot_base_height = abs(min_joint_z)  # Distance from current origin to bottom
        
#         print(f"Calculated robot base height from joints: {robot_base_height:.3f}m")
#         print(f"Lowest joint Z-coordinate: {min_joint_z:.3f}m")
#     else:
#         print(f"Using provided robot base height: {robot_base_height:.3f}m")
    
#     # Calculate horizontal center of the robot (X and Y)
#     # Use the centroid of joint positions as the robot's center
#     robot_center_x = np.mean(joint_origins_array[:, 0])
#     robot_center_y = np.mean(joint_origins_array[:, 1])
    
#     print(f"Robot center (X, Y): ({robot_center_x:.3f}, {robot_center_y:.3f})")
    
#     # Define the transformation to world coordinates
#     # World origin = bottom center of robot on the ground
#     world_transform = np.array([
#         -robot_center_x,  # Shift X to center the robot
#         -robot_center_y,  # Shift Y to center the robot  
#         robot_base_height # Shift Z so robot base is at ground level (Z=0)
#     ])
    
#     # Apply transformation
#     world_points = points + world_transform
    
#     # Also transform joint origins for reference
#     world_joint_origins = joint_origins_array + world_transform
    
#     transformation_info = {
#         'world_transform': world_transform,
#         'robot_base_height': robot_base_height,
#         'robot_center_xy': [robot_center_x, robot_center_y],
#         'original_bounds': {
...
#         },
#         'world_bounds': {
#             'x': [world_points[:, 0].min(), world_points[:, 0].max()],
#             'y': [world_points[:, 1].min(), world_points[:, 1].max()],
#             'z': [world_points[:, 2].min(), world_points[:, 2].max()]
#         },
#         'world_joint_origins': world_joint_origins
#     }
    
#     print(f"World coordinate transformation applied:")
#     print(f"  Translation vector: {world_transform}")
#     print(f"  Original point cloud bounds:")
#     print(f"    X: [{transformation_info['original_bounds']['x'][0]:.3f}, {transformation_info['original_bounds']['x'][1]:.3f}]")
#     print(f"    Y: [{transformation_info['original_bounds']['y'][0]:.3f}, {transformation_info['original_bounds']['y'][1]:.3f}]")
#     print(f"    Z: [{transformation_info['original_bounds']['z'][0]:.3f}, {transformation_info['original_bounds']['z'][1]:.3f}]")
#     print(f"  World coordinate bounds:")
#     print(f"    X: [{transformation_info['world_bounds']['x'][0]:.3f}, {transformation_info['world_bounds']['x'][1]:.3f}]")
#     print(f"    Y: [{transformation_info['world_bounds']['y'][0]:.3f}, {transformation_info['world_bounds']['y'][1]:.3f}]")
#     print(f"    Z: [{transformation_info['world_bounds']['z'][0]:.3f}, {transformation_info['world_bounds']['z'][1]:.3f}]")
    
#     return world_points, transformation_info

# def visualize_coordinate_comparison(robot_points, world_points, joint_origins, world_joint_origins, transformation_info):
#     """
#     Create a side-by-side visualization comparing robot and world coordinates
#     """
#     print("\n" + "="*60)
#     print("COORDINATE SYSTEM COMPARISON VISUALIZATION")
#     print("="*60)
    
#     fig = plt.figure(figsize=(20, 10))
    
#     # Plot 1: Robot coordinates (original)
#     ax1 = fig.add_subplot(121, projection='3d')
    
#     if len(robot_points) > 0:
#         distances_robot = np.linalg.norm(robot_points, axis=1)
#         scatter1 = ax1.scatter(robot_points[:, 0], robot_points[:, 1], robot_points[:, 2], 
#                               c=distances_robot, cmap='viridis', s=20, alpha=0.7)
#         plt.colorbar(scatter1, ax=ax1, label='Distance from Robot Origin (m)', shrink=0.5)
...
#     # Mark robot origin
#     ax1.scatter([0], [0], [0], c='blue', s=200, marker='x', label='Robot Origin [0,0,0]', linewidth=3)
    
#     ax1.set_title(f'Robot Coordinates ({len(robot_points)} points)')
#     ax1.set_xlabel('X (m)')
#     ax1.set_ylabel('Y (m)')
#     ax1.set_zlabel('Z (m)')
#     ax1.legend()
...
#     # Plot 2: World coordinates (transformed)
#     ax2 = fig.add_subplot(122, projection='3d')
    
#     if len(world_points) > 0:
#         distances_world = np.linalg.norm(world_points, axis=1)
#         scatter2 = ax2.scatter(world_points[:, 0], world_points[:, 1], world_points[:, 2], 
#                               c=distances_world, cmap='viridis', s=20, alpha=0.7)
#         plt.colorbar(scatter2, ax=ax2, label='Distance from World Origin (m)', shrink=0.5)
    
#     # Plot transformed joint origins
#     ax2.scatter(world_joint_origins[:, 0], world_joint_origins[:, 1], world_joint_origins[:, 2],
#                c='red', s=100, marker='o', label='Joint Origins', alpha=0.9)
    
#     # Mark world origin (bottom center of robot)
#     ax2.scatter([0], [0], [0], c='green', s=200, marker='x', label='World Origin [0,0,0]\n(Bottom Center)', linewidth=3)
    
#     # Add ground plane reference at Z=0 (using plot instead of plot_surface for legend compatibility)
#     if len(world_points) > 0:
#         x_range = [world_points[:, 0].min(), world_points[:, 0].max()]
#         y_range = [world_points[:, 1].min(), world_points[:, 1].max()]
#         # Create a grid outline instead of surface to avoid legend issues
#         ground_x = [x_range[0], x_range[1], x_range[1], x_range[0], x_range[0]]
#         ground_y = [y_range[0], y_range[0], y_range[1], y_range[1], y_range[0]]
#         ground_z = [0, 0, 0, 0, 0]
#         ax2.plot(ground_x, ground_y, ground_z, color='gray', linewidth=2, alpha=0.7, label='Ground Plane (Z=0)')
    
#     ax2.set_title(f'World Coordinates ({len(world_points)} points)')
#     ax2.set_xlabel('X (m)')
#     ax2.set_ylabel('Y (m)')
#     ax2.set_zlabel('Z (m)')
#     ax2.legend()
...
#     print(f"{'='*50}")
#     print(f"Transformation applied: {transformation_info['world_transform']}")
#     print(f"Robot base height: {transformation_info['robot_base_height']:.3f}m")
#     print(f"Robot center (X,Y): ({transformation_info['robot_center_xy'][0]:.3f}, {transformation_info['robot_center_xy'][1]:.3f})")
#     print(f"\nIn world coordinates:")
#     print(f"  - Origin [0,0,0] is at the bottom center of the robot on the ground")
#     print(f"  - Positive Z points upward from the ground")
#     print(f"  - Robot is centered at X=0, Y=0")
...
# world_points, transform_info = transform_to_world_coordinates(
#     point_positions, 
#     joint_origins,
#     robot_base_height=None  # Auto-calculate from joint positions
# )

# # Apply processing in world coordinates
# processed_world_points, processing_info = process_point_cloud_complete(
#     world_points,
...
# visualize_coordinate_comparison(
#     robot_points=point_positions,
#     world_points=world_points,
#     joint_origins=joint_origins, 
#     world_joint_origins=transform_info['world_joint_origins'],
#     transformation_info=transform_info
# )

# # Save world coordinate data
# np.save('world_coordinate_point_cloud.npy', world_points)
# np.save('processed_world_coordinate_point_cloud.npy', processed_world_points)