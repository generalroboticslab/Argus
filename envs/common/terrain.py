...
def discrete_obstacles_terrain(terrain: SubTerrain, max_height, min_size, max_size, num_rects, platform_size=1.):
    """
    Generate a terrain with gaps (Optimized Version)
    ...
    # 1. Vectorized generation of obstacle properties
    # Generate all rectangle sizes at once
    # Ensures sizes are a multiple of 4, matching the logic in the original code
    rect_widths = np.random.randint(min_size // 4, max_size // 4, size=num_rects) * 4
    rect_lengths = np.random.randint(min_size // 4, max_size // 4, size=num_rects) * 4

    # Generate all start positions at once, ensuring they are within bounds
    start_i = (np.random.rand(num_rects) * (rows - rect_widths)).astype(np.int32)
    start_j = (np.random.rand(num_rects) * (cols - rect_lengths)).astype(np.int32)
    
    # Align starting points to a grid of 4
    start_i = (start_i // 4) * 4
    start_j = (start_j // 4) * 4

    # Generate all heights at once
    rect_heights = np.random.choice(height_range, num_rects)

    # 2. Call the fast, JIT-compiled function to place obstacles
    _place_obstacles_jit(height_field, start_i, start_j, rect_widths, rect_lengths, rect_heights)

    # 3. Create the central platform (already efficient)
    x1 = (terrain.width - platform_size) // 2
    x2 = (terrain.width + platform_size) // 2
    y1 = (terrain.length - platform_size) // 2
    y2 = (terrain.length + platform_size) // 2
...
# Numba-compiled helper function to execute the loop at high speed
@numba.jit(nopython=True, cache=True)
def _place_obstacles_jit(height_field, start_i, start_j, widths, lengths, heights):
    """
    This function is compiled to machine code by Numba for maximum performance.
    """
    num_rects = len(heights)
    for k in range(num_rects):
        # Slice and assign the height for one obstacle
        height_field[start_i[k] : start_i[k] + widths[k], start_j[k] : start_j[k] + lengths[k]] = heights[k]
...
def convert_heightfield_to_trimesh(height_field_raw, horizontal_scale, vertical_scale, slope_threshold=None):
    ...
    if slope_threshold is not None:
        ...
        move_x = np.zeros_like(hf)
        move_y = np.zeros_like(hf)
        move_corners = np.zeros_like(hf)
    ...
    # Create triangle mesh vertices
    vertices = np.zeros((num_rows * num_cols, 3), dtype=np.float32)
    vertices[:, 0] = xx.flatten()
    vertices[:, 1] = yy.flatten()
    vertices[:, 2] = hf.flatten() * vertical_scale

    # Vectorized creation of the triangle array
    indices = np.arange(num_rows * num_cols, dtype=np.uint32).reshape(num_rows, num_cols)

    # Get the indices of the four corners of each quad in the grid
    ind0 = indices[:-1, :-1].flatten()
    ind1 = indices[:-1, 1:].flatten()
    ind2 = indices[1:, :-1].flatten()
    ind3 = indices[1:, 1:].flatten()

    # Create the two triangles for each quad
    num_quads = (num_rows - 1) * (num_cols - 1)
    triangles = np.empty((2 * num_quads, 3), dtype=np.uint32)
    
    # First triangle of each quad (ind0, ind3, ind1)
    triangles[0::2, 0] = ind0
    triangles[0::2, 1] = ind3
    triangles[0::2, 2] = ind1
    
    # Second triangle of each quad (ind0, ind2, ind3)
    triangles[1::2, 0] = ind0
    triangles[1::2, 1] = ind2
    triangles[1::2, 2] = ind3


    # # use open3d to remove degenerate triangles
    ...
    # # mesh = mesh.simplify_quadric_decimation(target_number_of_triangles=int(len(triangles)*0.9))
    # # Get the updated vertices and triangles
    # vertices = np.array(mesh.vertices, dtype=np.float32)
    # triangles = np.array(mesh.triangles, dtype=np.uint32)
    # vertical_sliver_area_threshold=1e-8  # Threshold for vertical sliver triangles based on their projected XY area
    # if slope_threshold is not None:
    #     # STAGE 1 & 2: Merge identical vertices and remove collapsed triangles
    #     unique_vertices_view, inverse_map = np.unique(
    #         vertices.view(np.dtype((np.void, vertices.dtype.itemsize * vertices.shape[1]))),
    #         return_inverse=True,
    #         axis=0
    #     )
    #     vertices = unique_vertices_view.view(vertices.dtype).reshape(-1, vertices.shape[1])
    #     remapped_triangles = inverse_map[triangles]
    #     valid_triangles_mask = (remapped_triangles[:, 0] != remapped_triangles[:, 1]) & \
    #                            (remapped_triangles[:, 1] != remapped_triangles[:, 2]) & \
    #                            (remapped_triangles[:, 0] != remapped_triangles[:, 2])
    #     triangles = remapped_triangles[valid_triangles_mask]
    #     # STAGE 3: Remove vertical sliver triangles based on their projected XY area
    #     if triangles.shape[0] > 0:
    #         v0 = vertices[triangles[:, 0]]
    #         v1 = vertices[triangles[:, 1]]
    #         v2 = vertices[triangles[:, 2]]
            
    #         # Calculate 2D area of the triangle's footprint on the XY plane
    #         # Using the vectorized Shoelace formula
    #         area_2d = 0.5 * np.abs(
    #             v0[:, 0] * (v1[:, 1] - v2[:, 1]) +
    #             v1[:, 0] * (v2[:, 1] - v0[:, 1]) +
    #             v2[:, 0] * (v0[:, 1] - v1[:, 1])
    #         )
    #         # Keep only triangles with a footprint area larger than the threshold
    #         sliver_mask = area_2d > vertical_sliver_area_threshold
    #         triangles = triangles[sliver_mask]
    # # Ensure final dtype is correct for Isaac Gym API
...
class Terrain:
    def __init__(self, cfg: Mapping, num_robots: int, device: Union[torch.device, str], gym, sim) -> None:
        ...
        elif self.terrain_type == "diy":
            #diy
            file_path = cfg.get("file_path", "../assets/urdf/climbup/parallel_rough_walls.obj")
            # check extension name is .obj or .npz
            t1 = time.time()
            print(f"Loading terrian mesh from:{file_path}")
            if file_path.endswith(".obj"):
                mesh = trimesh.load(file_path)
                # Convert to lists for Isaac Gym
                self.vertices = np.array(mesh.vertices,dtype=np.float32)
                self.triangles = np.array(mesh.faces,dtype=np.uint32)
            elif file_path.endswith(".npz"):
                saved_data = np.load(file_path)
                self.vertices = saved_data["vertices"].astype(np.float32)
                self.triangles = saved_data["faces"].astype(np.uint32)
            t2 = time.time()
            print(f"Time taken to load mesh: {t2-t1:.2f} seconds")
            params = gymapi.TriangleMeshParams()
            params.nb_vertices = self.vertices.shape[0]
            params.nb_triangles = self.triangles.shape[0]
            set_common_params(params, offset=0)
            gym.add_triangle_mesh(sim, self.vertices.ravel(order='C'),self.triangles.ravel(order='C'), params)
    ...
    def curriculum(self, num_robots, num_terrains, num_levels, randomize):
        ...
        for j in range(num_terrains):
            for i in range(num_levels):
                ...
                # uniform_height = self.uniform_height * self.difficulty_scale 
